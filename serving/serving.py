...
class Serving:
    def __init__(self, bot):
        ...
        self._latest_message_check_wait_limit = self.settings['serve_interval_maximum'] * 2
        self.next = None

    async def _save_scores(self):
        dataIO.save_json('data/serving/scores.json', self.scores)

    async def _save_subscriptions(self):
        dataIO.save_json('data/serving/subscriptions.json', self.subscriptions)

    async def _save_settings(self):
        dataIO.save_json('data/serving/settings.json', self.settings)

    @commands.group(pass_context=True, no_pm=True, name='serving')
    async def _serving(self, context):
        """Serving, food for you"""
        if context.invoked_subcommand is None:
            await send_cmd_help(context)

    @_serving.command(pass_context=True, no_pm=True, name='start')
    async def _start(self, context):
        """Start the serve"""
        server = context.message.server
        channel = context.message.channel
        if server.id in self.subscriptions:
            message = '**We\'re already serving!**'
        else:
            self.subscriptions[server.id] = channel.id
            message = '**The serve has started. Good luck to all.**'
            await self._save_subscriptions()
        await self.bot.say(message)

    @_serving.command(pass_context=True, no_pm=True, name='stop')
    async def _stop(self, context):
        """Stop the serving"""
        server = context.message.server
        if server.id not in self.subscriptions:
            message = '**We\'re not serving!**'
        else:
            del self.subscriptions[server.id]
            message = '**The serve has stopped.**'
            await self._save_subscriptions()
        await self.bot.say(message)

    @_serving.command(no_pm=True, name='timing')
    @checks.is_owner()
    async def _timing(self, interval_min: int, interval_max: int, bang_timeout: int):
        ...
        if interval_min > interval_max:
            ...
            self.settings['serve_interval_minimum'] = interval_min
            self.settings['serve_interval_maximum'] = interval_max
            self.settings['wait_for_bang_timeout'] = bang_timeout
            await self._save_settings()
            message = '**Timing has been set.**'
        await self.bot.say(message)

    @_serving.command(no_pm=True, name='next')
    @checks.is_owner()
    async def _next(self):
        ...
        if self.next:
            ...
            message = '**There is currently no serve.**'
        await self.bot.say(message)

    @_serving.command(pass_context=True, no_pm=True, name='score')
    async def _score(self, context, member: discord.Member):
        """This will show the score of a eating"""
        server = context.message.server
    ...
    @_serving.command(pass_context=True, no_pm=True, name='leaderboard', aliases=['scores'])
    async def _servingboard(self, context):
    ...
    async def _serving_loop(self):
        while self == self.bot.get_cog('Serving'):
            wait_time = random.randrange(self.settings['serve_interval_minimum'], self.settings['serve_interval_maximum'])
...
def check_folder():
    if not os.path.exists('data/serving'):
        print('Creating data/serving folder...')
        os.makedirs('data/serving')


def check_files():
    f = 'data/serving/settings.json'
    if not dataIO.is_valid_json(f):
        print('Creating empty settings.json...')
        data = {}
        data['serve_interval_minimum'] = 300
        data['serve_interval_maximum'] = 600
        data['wait_for_bang_timeout'] = 30
        dataIO.save_json(f, data)

    f = 'data/serving/subscriptions.json'
    if not dataIO.is_valid_json(f):
        print('Creating empty subscriptions.json...')
        dataIO.save_json(f, {})

    f = 'data/serving/scores.json'
    if not dataIO.is_valid_json(f):
...
def setup(bot):
    ...
    loop.create_task(cog._serving_loop())
    bot.add_cog(cog)